...
=========================================================================================
Integrated version with built-in ESRGAN and LapSRN models
"""
...
# ===================================================================
# SECTION 0: CORE ARCHITECTURES (SwinIR + ESRGAN + LapSRN)
# ===================================================================
...
class SatelliteSuperResolutionApp:
    ...
    def train_model(self, data_path, epochs, batch_size, patch_size, output_path, model_type='swinir'):
        ...
        try:
            ...
            if model_type == 'swinir':
                ...
                st.error("LapSRN cannot be trained through this interface")
                return False
        ...
        try:
            ...
            if method == "Deep Learning":
                if model_type == 'swinir':
                    ...
                    if not self.lapsrn_model:
                        self.lapsrn_model = self.load_model(model_path, 'lapsrn')
                        
                    if self.lapsrn_model:
                        with st.spinner("Applying LapSRN super-resolution..."):
                            try:
                                # Convert to BGR format expected by OpenCV
                                img_bgr = cv2.cvtColor(img1, cv2.COLOR_RGB2BGR)
                                sr_img = self.lapsrn_model.upsample(img_bgr)
                                sr_img = cv2.cvtColor(sr_img, cv2.COLOR_BGR2RGB)
                            except Exception as e:
                                st.error(f"LapSRN SR failed: {str(e)}")
                                st.warning("Falling back to bicubic interpolation")
                                sr_img = self.apply_classical_sr(img1, scale_factor)
            else:
    ...
    def run(self):
        ...
        st.markdown("""
            ...
            - **Deep Learning**: Our advanced Dual-SwinIR, ESRGAN or LapSRN models
        ...
        with st.sidebar:
            ...
            if sr_method == "Deep Learning":
                model_type = st.selectbox(
                    "Model Architecture",
                    ["SwinIR", "ESRGAN", "LapSRN"])
                    
                model_path = st.text_input(
                    "Model Path",
                    value=f"models/{model_type.lower()}_model.pb" if model_type == "LapSRN" else f"models/{model_type.lower()}_model.pth",
