...
class SatelliteSuperResolutionApp:
    ...
    def calculate_metrics(self, ref_img, test_img):
        ...
        # BRISQUE (using image-quality library)
        try:
            metrics['BRISQUE'] = iqm.brisque(test_img)
        except Exception as e:
            st.warning(f"Could not calculate BRISQUE: {str(e)}")
            metrics['BRISQUE'] = None
            
        # NIQE
        try:
            metrics['NIQE'] = iqm.niqe(test_img)
        except Exception as e:
            st.warning(f"Could not calculate NIQE: {str(e)}")
            metrics['NIQE'] = None
    ...
    def process_images(self, image1, image2, method, model_path=None, scale_factor=2):
        ...
        col1.image(img1, caption="Input Image 1", use_column_width=True)
        col2.image(img2, caption="Input Image 2", use_column_width=True)
        col3.image(img2_reg, caption="Registered Image 2", use_column_width=True)
        ...
        # Display results
        st.image(sr_img, caption=f"Super-Resolution Output ({method})", use_column_width=True)
